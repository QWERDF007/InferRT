...
该测试比较 `irt::features::v1::ShapeTemplateMatcher`（AVX2）与
...
v0 保持参考 `shape_based_matching` 风格的标量响应图与逐窗口评分。v1 将评分下沉到 AVX2 kernel：相邻候选批量累加、每 4 个特征以理论上界早停、按源图稀疏响应排序特征，并直接从量化标签用 shuffle 查表。96 features、方向容差 1 的常用配置满足 8-bit 累加上界，v1 因此一次处理 32 个候选；更大配置安全回退到 16-bit 或标量路径。多模板匹配仍按 `max_parallelism` 并行扫描（`0` 为自动，`1` 为串行），合并后再执行统一的稳定排序和 NMS。
