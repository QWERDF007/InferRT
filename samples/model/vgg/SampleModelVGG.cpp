...
int main(int argc, char *argv[])
{
    try
    {
        if (argc < 4 || argc > 5)
        {
            std::cerr << "Usage: " << argv[0] << " <vgg11|vgg13|vgg16|vgg19> <weights_file.wts> <image_path> [label_file]"
                      << std::endl;
            std::cerr << "Example: " << argv[0] << " vgg11 vgg11.wts dog.jpg" << std::endl;
            std::cerr << "         " << argv[0] << " vgg16 vgg16.wts dog.jpg imagenet1000_clsidx_to_labels.txt"
                      << std::endl;
            return -1;
        }
