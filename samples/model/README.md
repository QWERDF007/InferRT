...
```bash
cmake --build build --config Release --target inferrt_sample_classification
cmake --build build --config Release --target inferrt_sample_detection
cmake --build build --config Release --target inferrt_sample_onnx
cmake --build build --config Release --target inferrt_sample_sam
...
导出格式和更多用法示例见 [`feature_extract/README.md`](../features/feature_extract/README.md)。

## Faiss 图像搜索示例（详见 samples/features）
...
详见 [`image_search/README.md`](../features/image_search/README.md)。
