...
void AlexNet::buildFeatureNetwork(nvinfer1::INetworkDefinition *network, const WeightsMap &weights_map)
{
    using namespace nvinfer1;
    ...
    conv1->setPaddingNd(DimsHW{2, 2});
    named_tensors["conv1"] = conv1->getOutput(0);
    if (tryMarkFeatureOutputTensors(network, named_tensors))
    {
        return;
    }
    ...
    throw irt::Exception(Status::ERROR_INVALID_ARGUMENT, "Requested feature tensor is not available in AlexNet");
}
