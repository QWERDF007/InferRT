...
nvinfer1::ITensor *addYOLOv5Bottleneck(nvinfer1::INetworkDefinition *network, const WeightsMap &weights_map,
                                       nvinfer1::ITensor &input, int channels, bool shortcut,
                          ...
                          const std::string &prefix)
{
    ...
    auto *split0 = network->addSlice(*cv1, nvinfer1::Dims4{0, 0, 0, 0},
                                     nvinfer1::Dims4{dims.d[0], hidden, dims.d[2], dims.d[3]},
                                     nvinfer1::Dims4{1, 1, 1, 1});
    auto *split1 = network->addSlice(*cv1, nvinfer1::Dims4{0, hidden, 0, 0},
                                     nvinfer1::Dims4{dims.d[0], hidden, dims.d[2], dims.d[3]},
                                     nvinfer1::Dims4{1, 1, 1, 1});
                           ...
                           const std::string &prefix)
{
    ...
    auto *pool2
        = network->addPoolingNd(*pool1->getOutput(0), nvinfer1::PoolingType::kMAX,
                                nvinfer1::DimsHW{kernel_size, kernel_size});
    auto *pool3
        = network->addPoolingNd(*pool2->getOutput(0), nvinfer1::PoolingType::kMAX,
                                nvinfer1::DimsHW{kernel_size, kernel_size});
                                      ...
                                      const nvinfer1::Dims4 &input_shape)
{
    ...
    auto *box0 = addConvBnSiLU(network, weights_map, input, in_channels, 3, 1,
                               head_prefix + ".cv2." + index + ".0");
    auto *box1 = addConvBnSiLU(network, weights_map, *box0, in_channels, 3, 1,
                               head_prefix + ".cv2." + index + ".1");
    auto *box = addLinearConv1x1(network, weights_map, *box1, kYoloV8BoxChannels,
                                 head_prefix + ".cv2." + index + ".2");

    auto *cls0 = addConvBnSiLU(network, weights_map, input, class_channels, 3, 1,
                               head_prefix + ".cv3." + index + ".0");
    auto *cls1 = addConvBnSiLU(network, weights_map, *cls0, class_channels, 3, 1,
                               head_prefix + ".cv3." + index + ".1");
    auto *cls = addLinearConv1x1(network, weights_map, *cls1, num_classes,
                                 head_prefix + ".cv3." + index + ".2");
    ...
    std::array<nvinfer1::ITensor *, 2> cat_inputs{dfl, cls_shuffle->getOutput(0)};
    auto                              *cat = network->addConcatenation(cat_inputs.data(),
                                                                        static_cast<int32_t>(cat_inputs.size()));
    cat->setAxis(1);
...
void YOLOModelBase::normalizeModelConfig(IModelConfig &config) const
{
    if (config.inputShapes().size() == 1)
    {
        ...
        const bool is_default_shape = shape.d[0] == 1 && shape.d[1] == 3 && shape.d[2] == kDefaultImageSize
...
void YOLOModelBase::validateDetectionConfig() const
{
    ...
    if (shape.nbDims != 4 || shape.d[0] <= 0 || shape.d[1] != 3 || shape.d[2] <= 0 || shape.d[3] <= 0)
    {
        throw irt::Exception(Status::ERROR_INVALID_ARGUMENT,
                             "YOLO input shape must be NCHW with 3 channels, got [%d,%d,%d,%d]", shape.d[0],
...
void YOLOv5Detector::buildNetwork(nvinfer1::INetworkDefinition *network, const WeightsMap &weights_map)
{
    ...
    auto *sppf9 = addSPPF(network, weights_map, *c3_8, scaledWidth(1024, width), scaledWidth(1024, width), 5, "model.9");
    ...
    auto *c3_13 = addC3(network, weights_map, *cat12, scaledWidth(512, width), scaledDepth(3, depth), false, "model.13");
    ...
    auto *c3_17 = addC3(network, weights_map, *cat16, scaledWidth(256, width), scaledDepth(3, depth), false, "model.17");
    ...
    auto *c3_20 = addC3(network, weights_map, *cat19, scaledWidth(512, width), scaledDepth(3, depth), false, "model.20");
    ...
    auto *c3_23 = addC3(network, weights_map, *cat22, scaledWidth(1024, width), scaledDepth(3, depth), false, "model.23");
...
void YOLOv8Detector::buildNetwork(nvinfer1::INetworkDefinition *network, const WeightsMap &weights_map)
{
    ...
    const float depth = spec_.depth;
    const float width = spec_.width;
    const int   max_channels = spec_.max_channels;

    auto sw = [width, max_channels](int channels) { return scaledWidth(channels, width, max_channels); };
    ...
    const int box_branch_channels = width == 1.25F ? 80 : 64;
    const int class_branch_channels
        = width == 0.25F ? std::max(64, std::min(classes, 100)) : sw(256);

    auto *out0 = addDFLDetectBranch(network, weights_map, *c2f15, 0, "model.22", box_branch_channels,
                                    class_branch_channels, classes, shape);
    auto *out1 = addDFLDetectBranch(network, weights_map, *c2f18, 1, "model.22", box_branch_channels,
                                    class_branch_channels, classes, shape);
    auto *out2 = addDFLDetectBranch(network, weights_map, *c2f21, 2, "model.22", box_branch_channels,
                                    class_branch_channels, classes, shape);
