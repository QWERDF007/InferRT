...
{
    std::unique_ptr<cudaStream_t, decltype(StreamDeleter)> stream(new cudaStream_t{nullptr}, StreamDeleter);
...
typedef struct TensorRTParams
{
    ...
    /// TensorRT 日志对象。
    std::shared_ptr<TRTLogger> logger{nullptr};
