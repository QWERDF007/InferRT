                                        ...
                                        const WeightsMap &weights_map, int layer_index, int out_channels)
{
    ...
    IConvolutionLayer *conv   = network->addConvolutionNd(
        input, out_channels, DimsHW{3, 3}, weights_map.at(prefix + ".weight"), weights_map.at(prefix + ".bias"));
              ...
              const VGGConfig &config, bool feature_only = false)
{
    ...
    ITensor *fc1w = network
                        ->addConstant(DimsHW{4096, fc1_in_features},
                                      weights_map.at("classifier." + std::to_string(config.classifier_weight_indices[0])
                                                     + ".weight"))
                        ->getOutput(0);
    ...
    ITensor *fc2w = network
                        ->addConstant(DimsHW{4096, 4096},
                                      weights_map.at("classifier." + std::to_string(config.classifier_weight_indices[1])
                                                     + ".weight"))
                        ->getOutput(0);
    ...
    ITensor  *fc3w        = network
                        ->addConstant(DimsHW{num_classes, 4096},
                                      weights_map.at("classifier." + std::to_string(config.classifier_weight_indices[2])
                                                     + ".weight"))
                        ->getOutput(0);
    ...
    IMatrixMultiplyLayer *fc2_0
        = network->addMatrixMultiply(*fc1_2->getOutput(0), MatrixOperation::kNONE, *fc2w, MatrixOperation::kTRANSPOSE);
    ...
    IMatrixMultiplyLayer *fc3_0
        = network->addMatrixMultiply(*fc2_2->getOutput(0), MatrixOperation::kNONE, *fc3w, MatrixOperation::kTRANSPOSE);
