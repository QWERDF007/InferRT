                                                        ...
                                                        int min_features, float min_distance)
{
    ...
    for (;;)
    {
        ...
        buckets.reserve(irt::checkedSizeAdd(irt::checkedSizeMul(static_cast<size_t>(num_features), 2U,
                                                               "Shape matcher bucket capacity"),
                                           1U, "Shape matcher bucket capacity"));
        const float min_distance_sq = distance * distance;
                                         ...
                                         int max_results, bool use_spatial_index)
{
    ...
    cells.reserve(irt::checkedSizeAdd(irt::checkedSizeMul(matches.size(), 2U, "Shape matcher cell capacity"), 1U,
                                      "Shape matcher cell capacity"));
    for (const auto &match : matches)
    {
        bool keep = true;
...
{
    detail::validateShapeTemplateConfig(config_);
    if (!kernel_)
    {
    ...
    const std::vector<ShapeTemplateVariant> &variants)
{
    ...
    tasks.reserve(irt::checkedSizeMul(inputs.size(), variants.size(), "Shape matcher training task capacity"));
    ...
    const cv::Mat &image, float threshold, const cv::Mat &search_mask, ShapeTemplateMatchOptions options) const
{
    ...
    if (empty())
    {
        throw irt::Exception(irt::Status::INVALID_OPERATION, "ShapeTemplateMatcher has no templates");
    }
...
{
    detail::saveShapeTemplateYaml(template_file, config_, templates_);
}

void detail::ShapeTemplateMatcherEngine::load(const fs::path &template_file)
{
    detail::loadShapeTemplateYaml(template_file, config_, templates_, kernel_.get());
}
