...
inline PatchTokenGrid inferPatchTokenGrid(int64_t token_count, int input_height, int input_width)
{
    ...
    {
        throw irt::Exception(irt::Status::ERROR_INVALID_ARGUMENT, "Invalid patch token count for LegacyRoiFeatureExtractor");
    }
 ...
 */
class LegacyRoiFeatureExtractorImpl
{
    /**
     * @brief ROI 特征张量布局。
     */
...
public:
    LegacyRoiFeatureExtractorImpl(const RoiFeatureConfig &config, const std::filesystem::path &weights_file)
        : config_(config)
    ...
    std::vector<float> extractItems(const std::vector<RoiFeatureItem> &items)
    {
        ...
        {
            if (!item.polygon.empty()) throw irt::Exception(irt::Status::ERROR_INVALID_ARGUMENT, "Legacy ROIAlign supports rectangles only");
...
private:
                                           ...
                                           const std::vector<size_t> &image_indices = {}) const
    {
        ...
        for (size_t i = 0; i < rois.size(); ++i)
        {
            ...
            const auto geometry = irt::resolvePreprocessGeometry(image_extractor_.preprocessSpec(),
                                                                  mapped_image_size.width, mapped_image_size.height);
            const float sx = float(geometry.resized_width) / mapped_image_size.width;
            const float sy = float(geometry.resized_height) / mapped_image_size.height;
            const float ox = float(geometry.pad_left - geometry.crop_left);
            const float oy = float(geometry.pad_top - geometry.crop_top);
            const float fx = feature_width / image_extractor_.inputWidth();
            const float fy = feature_height / image_extractor_.inputHeight();
            mapped_rois[offset] = static_cast<float>(image_index);
            mapped_rois[offset+1] = (rois[i].x1*sx+ox)*fx;
            mapped_rois[offset+2] = (rois[i].y1*sy+oy)*fy;
            mapped_rois[offset+3] = (rois[i].x2*sx+ox)*fx;
            mapped_rois[offset+4] = (rois[i].y2*sy+oy)*fy;
        }
...
class LegacyRoiFeatureExtractor::Impl final : public LegacyRoiFeatureExtractorImpl
{
public:
    using LegacyRoiFeatureExtractorImpl::LegacyRoiFeatureExtractorImpl;
};

LegacyRoiFeatureExtractor::LegacyRoiFeatureExtractor(const RoiFeatureConfig &config, const std::filesystem::path &weights_file)
    : impl_(std::make_unique<Impl>(config, weights_file))
{
}

LegacyRoiFeatureExtractor::~LegacyRoiFeatureExtractor() = default;

LegacyRoiFeatureExtractor::LegacyRoiFeatureExtractor(LegacyRoiFeatureExtractor &&) noexcept = default;

LegacyRoiFeatureExtractor &LegacyRoiFeatureExtractor::operator=(LegacyRoiFeatureExtractor &&) noexcept = default;

int LegacyRoiFeatureExtractor::featureDim() const
{
    return impl_->featureDim();
}

size_t LegacyRoiFeatureExtractor::maxBatchSize() const noexcept
{
    return impl_->maxBatchSize();
}

std::vector<float> LegacyRoiFeatureExtractor::extract(const RoiFeatureItem &item)
{
    return impl_->extract(item);
}

std::vector<float> LegacyRoiFeatureExtractor::extractBatch(const std::vector<RoiFeatureItem> &items, size_t begin,
                                                     size_t count)
{
    return impl_->extractBatch(items, begin, count);
}

std::vector<float> LegacyRoiFeatureExtractor::extractItems(const std::vector<RoiFeatureItem> &items)
{
    return impl_->extractItems(items);
}

std::vector<float> LegacyRoiFeatureExtractor::extractAll(
