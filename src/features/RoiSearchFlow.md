...
3. 调用 `IModel::forwardFeatures()` 输出指定 `feature_name` 的空间特征图。
4. 校验输出特征图必须是 NCHW，且包含空间维度。
5. 将原图 ROI 映射到特征图坐标：
...
6. 构造 ROIAlign 输入 `[batch_index, mapped_x1, mapped_y1, mapped_x2, mapped_y2]`。
7. 调用 `irt::ops::RoIAlign`，输出形状为 `C x pooled_height x pooled_width`。
8. 将 ROIAlign 输出展平为一维向量。
9. 按 `config.norm` 执行 L1、L2 或 None 归一化。
...
2. 根据输出特征图通道数和 ROIAlign 输出尺寸确定 `feature_dim`。
3. 调用 `buildConfiguredFaissIndex()` 构建 Faiss 索引。
4. Faiss 构建过程中通过回调按 ROI 条目下标提取特征：
   - 单条回调：`extract(gallery_items[index])`
   - 连续批量回调：`extractBatch(gallery_items, begin, count)`
   - 任意下标批量回调：`extractBatch(gallery_items, indices)`
5. 写入 ROI 映射文件 `<index>.rois.txt`。
6. 写入元数据文件 `<index>.meta.txt`。
7. 保存或加载完成后的 Faiss 索引进入可查询状态。
...
8. 使用 `.rois.txt` 中的映射将 Faiss ID 还原为 `RoiSearchResult`。
9. 返回按相似度从高到低排序的结果列表。
...
- `TrainingFeatures`：抽样提取 ROI 训练特征。
...
- `feature_name` 应选择保留空间维度的特征图；如果输出是 CLS token 或纯向量，ROI 检索会拒绝构建。
