...
阅读顺序：先看本页的拓扑和 SSOT，再按路径回源码/CMake；需要 DINO 细节时读 [`02_spec.md`](dino_region_search_v1/02_spec.md)，需要验收矩阵时读 [`03_test_acceptance.md`](dino_region_search_v1/03_test_acceptance.md)，需要当前实测时读 `WORKLOG.md` 的具体日期标题。
...
- **DINO 区域检索**：公共 [`DinoRegionSearch.hpp`](../src/features/include/inferrt/features/DinoRegionSearch.hpp)，实现入口 [`DinoRegionSearch.cpp`](../src/features/DinoRegionSearch.cpp) 与 [`priv/dino`](../src/features/priv/dino)，测试为 [`TestDinoRegionSearch.cpp`](../tests/features/TestDinoRegionSearch.cpp)。DINO 使用自己的连续 descriptor generation/index。

DINO 不复用 Faiss ANN：其 index store 保存连续 descriptor 数组，scan 做分块穷举；Faiss 仍是 features target 的整体构建依赖。DINO 的公共 facade 负责 `build/update/search/evaluate/describeIndex/pruneGenerations`（CLI 别名为 `describe`/`prune`）和 JSON/扫描 backend 转发；[`DinoEngine.cpp`](../src/features/priv/dino/DinoEngine.cpp) 编排 ingest、views、backbone、descriptors、query、scan、fusion、fine match、storage、阈值和状态。
...
- **图库 build**：图像收集/解码/内容身份 → 多尺度 views → backbone patch grid → 区域窗口池化与局部叶节点 → FP32 语义合并/INT8 编码 → generation staging → manifest/摘要校验 → 原子切换 active generation。
- **增量 update**：读取 active generation，按内容摘要复用未变图的切片，重建新增/变化图、offsets 和摘要，再发布新 generation。
- **查询 search**：查询图与 ROI canonical 化 → query context views/backbone → 区域通道与局部通道分块 scan → view/window 重打分 → 双路配额融合和空间去重 → 按来源图分组批量 fine match → 同图 NMS、可选判定阈值和结果状态。
- **evaluate**：逐查询消费 benchmark manifest，以配置的粗选覆盖、最终一对一匹配、无匹配误检和门槛计算报告；未测门槛必须保持 `unverified`。

模块入口和边界见 [`DinoRegionSearchFlow.md`](../src/features/DinoRegionSearchFlow.md)；具体坐标、存储、状态和阈值语义见 [`02_spec.md`](dino_region_search_v1/02_spec.md)，不能在此复制一套字段表。
